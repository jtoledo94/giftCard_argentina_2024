...
const crearModal = (list) => {
  try {
    list.forEach((element) => {
      ...
      if (element.modalfooter) {
        const divFooter = document.createElement("div");
        divFooter.classList.add("container", "contenedor_modal_texto");
        element.modalfooter.forEach(linea => {
          if (linea.icon.includes("fa-gift")) {
            const p = document.createElement("p");
            p.classList.add("poppins-regular", "black");

            const whatsappLink = '<a href="https://wa.link/ujxlug" target="_blank"  text-decoration:none;">+54 9 11 6411-9465</a>';
            const textoTransformado = linea.texto.replace('[whatsapp]', whatsappLink);

            p.innerHTML = `<i class="${linea.icon}"></i>${textoTransformado}`;
